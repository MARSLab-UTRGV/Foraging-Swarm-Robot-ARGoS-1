...
for coords in results[:-1]:
	...
	print("nest location =[",xy[0],", ",xy[1], "]")	if(varyCapacity):
	    quantity = 4
            ...
            print("distance=", distance)            quantity = math.ceil((forageRate*2*distance*branch**(level-idx-1))/speed)
            print("measured quantity=", quantity)            maxQuantity = round(distance/0.5)
            print("maxQuantity=", maxQuantity)            if quantity > maxQuantity:
                quantity = maxQuantity

        print("quantity", quantity)        delierying_robot += quantity
